...
from microsub_client.outbound import UnsafeOutboundURLError
...
@override_settings(STORAGES=SIMPLE_STORAGES)
class PhotoProxyViewTests(TestCase):
    def _auth_session(self):
        session = self.client.session
        session.update(auth_session())
        session.save()
    ...
    @patch("microsub_client.views.safe_request")
    def test_streams_photo_bytes_with_upstream_content_type(self, mock_safe_request):
        self._auth_session()
        draft = self._draft_with_photo()
        photo_hash = photo_url_hash(draft.photos[0])
        ...
        response = self.client.get(f"/drafts/{draft.pk}/photo/{photo_hash}/raw/")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response["Content-Type"], "image/jpeg")
        self.assertEqual(response.content, b"fake-jpeg-bytes")
        mock_resp.raise_for_status.assert_called_once()
        mock_safe_request.assert_called_once()
        self.assertEqual(mock_safe_request.call_args.args[0], draft.photos[0])

    @patch("microsub_client.views.safe_request", side_effect=UnsafeOutboundURLError("blocked"))
    def test_unsafe_upstream_url_returns_502(self, _mock):
        self._auth_session()
        draft = self._draft_with_photo()
        photo_hash = photo_url_hash(draft.photos[0])
        response = self.client.get(f"/drafts/{draft.pk}/photo/{photo_hash}/raw/")
    ...
    def test_unknown_hash_returns_404(self):
        self._auth_session()
        draft = self._draft_with_photo()
        response = self.client.get(f"/drafts/{draft.pk}/photo/deadbeef/raw/")
        self.assertEqual(response.status_code, 404)

    def test_cannot_proxy_another_users_draft_photo(self):
        # This is the whole point of resolving the URL server-side from a
        # session-owned Draft rather than trusting a client-supplied URL —
        # confirm it actually can't be used as an open proxy for other drafts.
        self._auth_session()
        other_draft = Draft.objects.create(
            user_url="https://someone-else.example/",
            photos=["https://media.example/uploads/b.jpg"],
        ...
        photo_hash = photo_url_hash(other_draft.photos[0])
        response = self.client.get(f"/drafts/{other_draft.pk}/photo/{photo_hash}/raw/")
        self.assertEqual(response.status_code, 404)
